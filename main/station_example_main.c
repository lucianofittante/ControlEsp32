...
esp_err_t send_data(httpd_req_t *req) { //  This function send data to the website
    ...
    char json_buf[256]; // Adjust buffer size as needed
    snprintf(json_buf, sizeof(json_buf), "{\"temperature\": %d, \"humidity\": %d, \"humedadsuelo\": %d, \"regar\": %s, \"alarmariego\": %s, \"time\": \"%s\"}",
             temperature, humidity, humedadsuelo, regar ? "true" : "false", alarmariego ? "true" : "false", current_time);
...
void http_server_task(void *pvParameter)
{
        ...
        if (httpd_start(&server, &config) == ESP_OK)
        {
                ...
                if (wifi_mode == WIFI_MODE_AP) {
                    ...
                    httpd_uri_t send_datagraf_uri_get = {
                        .uri       = "/datagraf",
                        .method    = HTTP_GET,
                        .handler   = send_datagraf,
                        .user_ctx  = NULL
                    };
